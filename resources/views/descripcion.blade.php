        ...
        <div id="anuncio"
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg w-3/4 md:w-1/2 text-center">
                ...
                <p class="text-color-titulos-entrega mt-2">
                    ...
                    @else
                        @php
                            // Obtener el siguiente día hábil y su horario
                            $siguienteHorario = $empresa->horarios->where('dia', '>', $dia)->first();
                            if (!$siguienteHorario) {
                                // Si no hay más días en la semana, reiniciar desde el lunes
                                $siguienteHorario = $empresa->horarios->first();
                            }
                            $siguienteDia = ucfirst($siguienteHorario->dia);
                            $siguienteHoraApertura = \Carbon\Carbon::parse($siguienteHorario->hora_inicio)->format('h:i A');
                            $siguienteHoraCierre = \Carbon\Carbon::parse($siguienteHorario->hora_fin)->format('h:i A');
                        @endphp
