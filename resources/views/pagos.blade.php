@extends('layout-cuenta')
@section('logica')
    <div class="flex flex-col w-full h-screen">
        <b
            class="flex justify-center text-[30px] items-center font-cabin leading-[45px] tracking-normal text-color-titulos-entrega font-bold">
            Pagos de
            hoy: S/ 0.00</b>
        <!---Nombres de Productos-->
        <div class="flex justify-center mt-3.5 space-x-7 text-wrap">
            @if ($usuario->tipo !== 'cliente')
                @if ($usuario->empresa && $usuario->empresa->productos->isNotEmpty())
                    @foreach ($usuario->empresa->productos as $item)
                        <p class="font-sans text-base text-wrap"><b class="m-1">0</b>{{ $item->descripcion }}.</p>
                    @endforeach
                @else
                    <p class="font-sans text-base text-wrap">Sin Productos Registrados en la Distribuidora.</p>
                @endif
            @endif
        </div>

        <div class="flex items-center justify-center  space-x-6 mt-5">
            <div class="flex items-center">
                <div class="w-[40px] h-[40px]"><svg xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="40px" height="40px"
                        viewBox="0 0 83.432 57.333" xml:space="preserve">
                        <g id="OBJECTS">
                            <g>
                                <path fill="#FFDAB6"
                                    d="M3.456,21.349c-5.67,3.509-3.416,23.198,0,22.97c4.851,2.128,1.781,4.313,9.783,7.798   c4.426,1.937,8.886,2.659,13.564,2.911c1.802,0.098,3.647,0.127,5.527,0.127c2.202,0,4.46-0.043,6.804-0.049   c0.392-0.007,0.785-0.007,1.177-0.007h22.915c1.69,0,3.064-1.374,3.064-3.065c0-1.697-1.375-3.072-3.064-3.072H46.197   c0.357-0.849,0.694-1.761,1.002-2.721h28.595c1.844,0,3.331-1.494,3.331-3.339c0-1.844-1.487-3.338-3.331-3.338H48.854   c0.056-0.288,0.105-0.582,0.155-0.877c0.098-0.533,0.182-1.065,0.252-1.599c0.056-0.337,0.104-0.667,0.147-1.003h30.235   c2.09,0,3.788-1.698,3.788-3.795c0-2.09-1.697-3.787-3.788-3.787H49.998c0.007-0.442,0.007-0.884,0-1.311   c0-0.281-0.007-0.561-0.021-0.835c0-0.167-0.007-0.336-0.014-0.497h21.413c2.09,0,3.787-1.698,3.787-3.788   c0-2.09-1.697-3.788-3.787-3.788H42.654c0,0-9.342,1.122-7.736-2.707c0.484-1.158,3.066-1.564,6.277-2.161   c0.976-0.182,2.013-0.378,3.066-0.631c1.283-0.294,2.58-0.659,3.836-1.15c0.077-0.021,0.154-0.056,0.231-0.084   c1.494-0.597,2.91-1.361,4.117-2.384c0.105-0.084,0.21-0.176,0.316-0.268c0.112-0.098,0.218-0.204,0.315-0.301   c5.184-5.092-2.005-9.006-5.73-5.113c-0.28,0.294-0.624,0.561-1.017,0.807c-0.793,0.491-1.803,0.877-2.959,1.192   c-1.824,0.491-4.026,0.8-6.368,1.017c-0.456,0.042-0.927,0.078-1.397,0.12c-1.479,0.112-2.994,0.203-4.495,0.294   c-1.466,0.084-2.925,0.168-4.307,0.273c-1.936,0.147-3.739,0.336-5.26,0.624C13.961,9.257,12.16,21.349,4.76,21.349   C4.76,21.349,3.686,21.119,3.456,21.349z">
                                </path>
                                <rect x="33.299" y="6.579" fill="#2FC280" width="21.525" height="50.754"></rect>
                                <path fill="#FFFFFF"
                                    d="M46.96,52.687h-5.798c0-2.15-1.743-3.892-3.892-3.892V15.119c2.149,0,3.892-1.743,3.892-3.892h5.798   c0,2.149,1.742,3.892,3.892,3.892v33.676C48.703,48.795,46.96,50.537,46.96,52.687z">
                                </path>
                                <circle fill="#2FC280" cx="44.061" cy="31.957" r="4.765"></circle>
                                <rect x="16.653" y="18.94"
                                    transform="matrix(-0.2872 0.9579 -0.9579 -0.2872 82.5523 -2.0255)" fill="#2FC280"
                                    width="50.754" height="21.524"></rect>
                                <path fill="#FFFFFF"
                                    d="M38.853,50.392l-5.554-1.665c0.617-2.06-0.551-4.229-2.611-4.846l9.674-32.258   c2.058,0.618,4.228-0.551,4.845-2.61l5.554,1.665c-0.618,2.059,0.551,4.229,2.609,4.846l-9.672,32.259   C41.64,47.164,39.47,48.333,38.853,50.392z">
                                </path>
                                <circle fill="#2FC280" cx="42.03" cy="29.702" r="4.766"></circle>
                                <rect x="14.628" y="15.84" transform="matrix(-0.623 0.7822 -0.7822 -0.623 85.7363 11.8812)"
                                    fill="#2FC280" width="50.753" height="21.525"></rect>
                                <path fill="#FFFFFF"
                                    d="M29.357,44.624l-4.535-3.611c1.339-1.682,1.062-4.131-0.621-5.47L45.182,9.2   c1.681,1.34,4.13,1.062,5.469-0.619l4.535,3.612c-1.339,1.681-1.062,4.129,0.62,5.469l-20.98,26.343   C33.144,42.666,30.696,42.942,29.357,44.624z">
                                </path>
                                <circle fill="#2FC280" cx="40.004" cy="26.603" r="4.766"></circle>
                                <circle fill="none" stroke="#F7931E" stroke-width="0.5414" stroke-miterlimit="10"
                                    cx="51.112" cy="30.504" r="8.121"></circle>
                                <circle fill="#FFD53D" cx="51.112" cy="30.504" r="8.121"></circle>
                                <circle fill="none" stroke="#F7931E" stroke-width="0.5414" stroke-miterlimit="10"
                                    cx="51.112" cy="30.503" r="6.269"></circle>
                                <circle fill="#FFD53D" cx="54.958" cy="32.523" r="8.121"></circle>
                                <circle fill="none" stroke="#F7931E" stroke-width="0.5414" stroke-miterlimit="10"
                                    cx="54.959" cy="32.523" r="8.121"></circle>
                                <circle fill="none" stroke="#F7931E" stroke-width="0.5414" stroke-miterlimit="10"
                                    cx="54.958" cy="32.523" r="6.269"></circle>
                                <path fill="#FFDAB6"
                                    d="M1.155,41.114l31.483-22.539c0.661-0.829,2-2.325,2.28-2.998c0.484-1.158,3.066-1.564,6.277-2.161   c0.976-0.182,2.013-0.378,3.066-0.631c1.283-0.294,2.58-0.659,3.836-1.15c0.077-0.021,0.154-0.056,0.231-0.084   c1.494-0.597,2.91-1.361,4.117-2.384c0.105-0.084,0.21-0.176,0.316-0.268c0.112-0.098,0.218-0.204,0.315-0.301   c5.184-5.092-2.005-9.006-5.73-5.113c-0.28,0.294-0.624,0.561-1.017,0.807c-0.793,0.491-1.803,0.877-2.959,1.192   c-1.824,0.491-4.026,0.8-6.368,1.017c-0.456,0.042-0.927,0.078-1.397,0.12c-1.479,0.112-2.994,0.203-4.495,0.294   c-1.466,0.084-2.925,0.168-4.307,0.273c-1.936,0.147-3.739,0.336-5.26,0.624C13.961,9.257,12.16,21.349,4.76,21.349   c0,0-1.073-0.231-1.304,0C-0.747,23.951-0.599,35.432,1.155,41.114z">
                                </path>
                            </g>
                        </g>
                        <g id="Capa_3" display="none">
                            <path display="inline" fill="#027FA5"
                                d="M86.929,74.907c0,6.532-5.295,11.827-11.827,11.827h-56.77  c-6.532,0-11.827-5.295-11.827-11.827V32.528c0-6.532,5.295-11.827,11.827-11.827h56.77c6.532,0,11.827,5.295,11.827,11.827V74.907  z">
                            </path>
                            <rect x="13.601" y="20.701" transform="matrix(0.9883 -0.1527 0.1527 0.9883 -4.595 6.9816)"
                                display="inline" fill="#2FC280" width="59.135" height="25.428"></rect>
                            <path display="inline" fill="#00ADEF"
                                d="M80.424,77.852c0,6.532-5.295,11.827-11.827,11.827h-56.77  C5.295,89.679,0,84.383,0,77.852v-42.38c0-6.532,5.295-11.827,11.827-11.827h56.77c6.532,0,11.827,5.295,11.827,11.827V77.852z">
                            </path>
                            <path display="inline" fill="#86BBD3"
                                d="M94.025,56.63c0,3.811-3.089,6.899-6.899,6.899h-28.78c-3.81,0-6.898-3.089-6.898-6.899  l0,0c0-3.81,3.089-6.898,6.898-6.898h28.78C90.936,49.732,94.025,52.821,94.025,56.63L94.025,56.63z">
                            </path>
                            <circle display="inline" fill="#FFD53D" cx="63.484" cy="56.63" r="3.955"></circle>
                        </g>
                        <g id="Capa_2"></g>
                    </svg></div>
                <div class="text-base">S/ 0.00</div>
            </div>
            <div class="flex items-center">
                <div class="w-[40px] h-[40px] "><svg xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink" id="Capa_1" x="0px" y="0px" width="40px"
                        height="40px" viewBox="0 0 83.432 105.811" xml:space="preserve">
                        <g id="Capa_3" display="none">
                            <path display="inline" fill="#027FA5"
                                d="M86.929,74.907c0,6.531-5.295,11.826-11.827,11.826h-56.77  c-6.532,0-11.827-5.295-11.827-11.826v-42.38c0-6.531,5.295-11.826,11.827-11.826h56.77c6.532,0,11.827,5.295,11.827,11.826V74.907  z">
                            </path>
                            <rect x="13.603" y="20.699" transform="matrix(-0.9883 0.1527 -0.1527 -0.9883 90.9383 59.8435)"
                                display="inline" fill="#2FC280" width="59.137" height="25.429"></rect>
                            <path display="inline" fill="#00ADEF"
                                d="M80.424,77.852c0,6.532-5.295,11.827-11.827,11.827h-56.77  C5.295,89.679,0,84.383,0,77.852v-42.38c0-6.532,5.295-11.827,11.827-11.827h56.77c6.532,0,11.827,5.295,11.827,11.827V77.852z">
                            </path>
                            <path display="inline" fill="#86BBD3"
                                d="M94.025,56.63c0,3.811-3.089,6.898-6.898,6.898h-28.78c-3.81,0-6.897-3.089-6.897-6.898  l0,0c0-3.811,3.089-6.898,6.897-6.898h28.78C90.936,49.732,94.025,52.821,94.025,56.63L94.025,56.63z">
                            </path>
                            <circle display="inline" fill="#FFD53D" cx="63.484" cy="56.63" r="3.955">
                            </circle>
                        </g>
                        <g id="Capa_2">
                            <g>
                                <path fill="#A221AF"
                                    d="M21.903,70.285c-0.6,0.571-1.171,1.117-1.758,1.647c-0.803,0.736-1.688,1.35-2.71,1.731   c-2.506,0.912-4.74-0.259-5.462-2.82c-0.313-1.131-0.272-2.275-0.082-3.419c0.204-1.2,0.38-2.412,0.681-3.583   c0.313-1.213,0.722-2.384,1.132-3.57c0.516-1.512,1.062-3.023,1.62-4.536c0.354-0.953,0.341-1.92,0.3-2.901   c-0.057-1.021,0.436-1.594,1.429-1.539c0.818,0.041,1.623,0.149,2.249,0.803c0.723,0.751,0.967,1.649,0.708,2.63   c-0.49,1.854-1.076,3.678-1.593,5.518c-0.422,1.498-0.817,3.011-1.172,4.536c-0.123,0.531-0.123,1.104-0.055,1.647   c0.108,1.063,0.98,1.607,1.947,1.157c0.558-0.259,1.089-0.667,1.54-1.116c1.389-1.403,2.192-3.174,2.846-5.013   c0.232-0.642,0.479-1.282,0.708-1.936c0.52-1.458,1.049-2.901,1.556-4.358c0.421-1.24,0.789-2.507,1.197-3.746   c0.683-2.002,2.807-3.706,4.973-3.598c1.117,0.057,1.376,0.532,1.172,1.473c-0.383,1.757-0.845,3.486-1.336,5.218   c-2.152,7.683-4.305,15.354-6.471,23.021c-0.04,0.162-0.081,0.34-0.121,0.489c7.505-1.225,14.996-2.438,22.559-3.664   c0.298-1.594,0.612-3.256,0.966-5.066c-0.3,0.189-0.477,0.299-0.653,0.408c-0.792,0.477-1.607,0.899-2.549,1.075   c-1.634,0.287-2.86-0.448-3.352-2.043c-0.067-0.218-0.121-0.448-0.203-0.734c-0.3,0.53-0.546,1.021-0.846,1.483   c-0.845,1.294-1.907,2.304-3.474,2.657c-2.302,0.503-4.55-0.695-5.366-2.916c-0.45-1.226-0.642-2.52-0.683-3.828   c-0.123-3.665,0.804-7.11,2.167-10.476c0.64-1.593,1.43-3.105,2.465-4.468c0.79-1.035,1.677-1.977,2.739-2.738   c1.716-1.227,4.237-1.43,6.253,0.423c0.122-0.368,0.245-0.708,0.354-1.063c0.285-0.912,1.021-1.172,1.961-1.049   c0.369,0.055,0.734,0.15,1.09,0.272c0.708,0.246,1.077,0.763,1.089,1.499c0,0.478-0.081,0.967-0.177,1.429   c-0.79,3.639-1.608,7.275-2.356,10.912c-0.23,1.159-0.271,2.358-0.38,3.543c-0.028,0.231,0.013,0.489,0.067,0.721   c0.135,0.587,0.407,0.736,0.967,0.504c0.409-0.162,0.791-0.381,1.159-0.625c0.585-0.382,0.871-0.912,1.008-1.623   C51,57.357,52.035,52.059,53.057,46.76c0.013-0.069,0.027-0.123,0.042-0.19c0.313-1.472,0.871-1.472,2.098-1.309   c0.449,0.055,0.886,0.204,1.321,0.354c0.545,0.19,0.682,0.45,0.626,1.035c-0.04,0.354-0.081,0.694,0,1.09   c0.313-0.368,0.613-0.749,0.927-1.116c0.408-0.463,0.764-0.981,1.239-1.35c1.295-1.007,2.793-1.279,4.389-0.885   c1.089,0.259,1.757,1.035,2.191,2.029c0.681,1.565,0.871,3.228,0.913,4.904c0.082,3.118-0.613,6.116-1.498,9.058   c-0.504,1.689-1.132,3.339-2.071,4.85c-0.504,0.817-1.063,1.637-1.717,2.344c-2.711,2.943-6.184,1.934-8.092,0.177   c-0.15-0.136-0.272-0.286-0.531-0.545c-0.341,2.274-0.653,4.414-0.995,6.661c0.642-0.068,1.171-0.137,1.719-0.203   c4.29-0.532,8.581-1.063,12.873-1.567c1.946-0.23,3.908-0.436,5.856-0.572c1.021-0.067,2.07-0.027,3.105,0.055   c1.662,0.136,2.397,1.729,2.043,3.119c-0.164,0.627-0.586,1.091-1.267,1.226c-1.186,0.232-2.371,0.504-3.569,0.655   c-5.313,0.68-10.641,1.319-15.952,1.975c-1.702,0.218-3.405,0.462-5.108,0.667c-0.368,0.041-0.517,0.163-0.613,0.531   c-0.162,0.654-0.38,1.308-0.626,1.948c-0.244,0.641-0.708,1.116-1.308,1.443c-1.036,0.572-2.193,0.041-2.356-1.131   c-0.095-0.667-0.014-1.348-0.014-2.098c-0.995,0.137-1.948,0.247-2.889,0.409c-6.361,1.116-12.696,2.315-18.989,3.802   c-0.532,0.122-1.198,0.135-1.554,0.475c-0.354,0.329-0.437,0.982-0.612,1.5c-0.968,2.957-2.044,5.871-3.474,8.638   c-1.539,2.982-3.394,5.733-5.83,8.05c-1.5,1.417-3.189,2.479-5.273,2.833c-3.678,0.612-6.784-1.825-7.042-5.558   c-0.123-1.771,0.354-3.446,0.913-5.108c0.721-2.111,1.797-4.032,3.147-5.79c0.708-0.926,1.471-1.852,2.354-2.628   c3.093-2.685,6.403-5.041,10.313-6.432c0.229-0.082,0.475-0.136,0.667-0.271c0.23-0.177,0.504-0.396,0.6-0.641   c0.98-2.902,1.919-5.816,2.874-8.718C21.93,70.421,21.903,70.34,21.903,70.285z M16.834,86.333   c-0.189,0.067-0.38,0.123-0.559,0.204c-2.86,1.39-5.325,3.269-7.287,5.789c-1.227,1.566-2.315,3.229-2.875,5.164   c-0.163,0.585-0.246,1.211-0.218,1.811c0.042,0.967,0.858,1.485,1.771,1.185c0.368-0.121,0.721-0.325,1.049-0.545   c1.021-0.681,1.825-1.594,2.561-2.561c1.702-2.207,3.024-4.646,4.155-7.193c0.543-1.239,0.98-2.534,1.457-3.813   c0.027-0.028,0.055-0.056,0.083-0.083C16.945,86.319,16.89,86.333,16.834,86.333z M62.292,52.481   c-0.08-0.462-0.136-1.239-0.354-1.962c-0.396-1.295-1.28-1.605-2.37-0.776c-0.612,0.479-1.198,1.076-1.58,1.729   c-0.6,1.05-1.09,2.167-1.484,3.298c-0.981,2.901-1.621,5.897-2.234,8.881c-0.083,0.382,0.014,0.653,0.354,0.817   c0.3,0.136,0.599,0.286,0.926,0.327c1.253,0.149,2.329-0.299,3.133-1.228c0.559-0.64,1.036-1.375,1.376-2.15   c0.531-1.199,0.954-2.452,1.363-3.719C61.925,56.118,62.184,54.47,62.292,52.481z M44.42,52.835   c0.025-1.321-0.682-1.894-1.715-1.634c-0.273,0.066-0.546,0.189-0.806,0.299c-1.512,0.681-2.588,1.812-3.404,3.215   c-1.199,2.043-1.894,4.263-2.125,6.605c-0.108,1.117-0.055,2.248,0.382,3.324c0.095,0.246,0.205,0.49,0.354,0.709   c0.736,1.049,1.867,1.13,2.751,0.219c0.791-0.831,1.282-1.854,1.636-2.916c0.845-2.521,1.621-5.067,2.396-7.614   C44.148,54.238,44.297,53.407,44.42,52.835z">
                                </path>
                                <path fill="#01E0C5"
                                    d="M48.575,0.975c7.355,0.45,13.173,3.842,16.359,11.007c3.188,7.181,1.036,15.652-5.041,20.665   c-3.854,3.174-8.281,4.578-13.254,4.318c-1.498-0.082-2.982-0.34-4.398-0.858c-0.695-0.258-1.296-0.189-1.881,0.205   c-0.885,0.585-1.758,1.17-2.643,1.757c-0.545,0.367-1.103,0.735-1.662,1.09c-0.189,0.123-0.381,0.219-0.585,0.313   c-0.926,0.408-1.349,0.165-1.39-0.83c-0.027-0.586,0.013-1.185,0.095-1.785c0.15-1.197,0.34-2.397,0.518-3.597   c0.163-1.007-0.069-1.838-0.776-2.655c-3.065-3.568-4.455-7.751-4.224-12.451c0.178-3.583,1.443-6.783,3.516-9.685   c0.612-0.845,1.361-1.608,2.125-2.343c2.997-2.89,6.619-4.454,10.719-4.987C46.777,1.043,47.499,1.043,48.575,0.975z    M44.312,28.015c0.723-0.163,1.676-0.271,2.548-0.599c1.839-0.682,2.833-2.533,2.493-4.399c-0.191-1.049-0.776-1.854-1.636-2.466   c-1.184-0.831-2.533-1.308-3.867-1.84c-0.696-0.271-1.405-0.572-1.99-1.021c-1.212-0.913-0.993-2.452,0.409-3.064   c1.158-0.503,2.289-0.34,3.352,0.3c0.669,0.394,1.063,0.981,1.076,1.784c0,0.177,0.041,0.354,0.082,0.518   c0.178,0.601,0.667,0.967,1.213,0.899c0.599-0.056,1.063-0.519,1.075-1.158c0.028-1.593-0.53-2.916-1.961-3.705   c-1.947-1.076-3.99-1.213-6.048-0.354c-1.212,0.504-2.015,1.417-2.179,2.765c-0.177,1.472,0.244,2.725,1.525,3.556   c0.693,0.449,1.457,0.818,2.221,1.172c0.749,0.354,1.552,0.626,2.303,0.995c0.489,0.245,0.966,0.544,1.362,0.912   c0.626,0.613,0.831,1.403,0.571,2.247c-0.245,0.818-0.98,1.009-1.717,1.131c-0.327,0.055-0.653,0.069-0.994,0.069   c-2.111-0.016-3.16-0.954-3.404-3.039c-0.097-0.749-0.574-1.199-1.268-1.144c-0.669,0.042-1.132,0.586-1.077,1.335   c0.163,2.166,1.35,4.155,3.869,4.781C42.854,27.852,43.467,27.893,44.312,28.015z M59.719,13.534   c-0.015-0.6-0.232-0.94-0.641-1.145c-0.396-0.189-0.914-0.108-1.227,0.218c-0.164,0.178-0.286,0.382-0.409,0.6   c-2.274,4.154-4.55,8.31-6.824,12.465c-0.109,0.19-0.219,0.381-0.301,0.585c-0.164,0.478,0.028,0.996,0.449,1.267   c0.423,0.272,0.981,0.246,1.35-0.108c0.149-0.149,0.272-0.34,0.381-0.531c2.329-4.25,4.659-8.5,6.976-12.75   C59.597,13.902,59.664,13.657,59.719,13.534z">
                                </path>
                                <path fill="#A221AF"
                                    d="M67.129,60.899c0.082-1.456,0.015-2.643,0.245-3.771c0.313-1.553,0.763-3.106,1.308-4.59   c0.926-2.563,2.152-4.987,3.896-7.099c0.857-1.022,1.865-1.962,2.94-2.725c1.173-0.831,2.588-0.954,4.021-0.775   c0.858,0.108,1.661,0.34,2.355,0.886c0.846,0.682,1.31,1.54,1.472,2.615c0.178,1.186-0.027,2.316-0.408,3.434   c-1.526,4.48-4.59,7.314-9.209,8.433c-0.067,0.013-0.137,0.054-0.19,0.041c-0.886-0.123-1.09,0.476-1.294,1.145   c-0.287,0.993-0.423,1.975-0.177,2.996c0.271,1.104,1.13,1.689,2.274,1.513c1.279-0.19,2.371-0.845,3.338-1.662   c0.791-0.667,1.525-1.402,2.22-2.166c1.021-1.145,1.623-1.076,2.248,0.34c0.042,0.095,0.083,0.205,0.136,0.299   c0.64,1.336,0.438,2.426-0.667,3.501c-1.785,1.717-3.598,3.379-5.939,4.292c-0.858,0.327-1.758,0.627-2.671,0.708   c-2.506,0.246-4.453-1.076-5.299-3.474c-0.164-0.45-0.299-0.926-0.367-1.403C67.251,62.508,67.197,61.568,67.129,60.899z    M73.544,52.985c0.042,0.055,0.083,0.109,0.123,0.179c0.586-0.22,1.2-0.382,1.757-0.667c1.607-0.817,2.944-1.936,3.597-3.707   c0.205-0.544,0.327-1.104,0.149-1.688c-0.162-0.586-0.626-0.913-1.198-0.734c-0.423,0.123-0.872,0.354-1.185,0.667   C75.139,48.693,74.226,50.778,73.544,52.985z">
                                </path>
                            </g>
                        </g>
                    </svg></div>
                <div class="text-base">S/ 0.00</div>
            </div>
            <div class="flex items-center">
                <div class="w-[30px] h-[40px] ">
                    <i class="fas fa-clock text-red-500 text-2xl"></i>
                </div>
                <div class="text-base">S/ 0.00</div>
            </div>



        </div>

        <div class="flex justify-center items-center mt-5 space-x-6">
            <div class="grid space-y-2">
                <label for="medio" class="ml-2">
                    Medio
                </label>
                <select name="medio" id="medio"
                    class="w-[138px] h-[50px] rounded-[20px] p-2 border border-color-text">
                    <option value="" selected>Seleccione...</option>
                    <option value="yape">Pago con Yape</option>
                    <option value="efectivo">Pago en Efectivo</option>
                    <option value="cuenta">Pago Pendiente</option>

                </select>
            </div>
            <div class="grid space-y-2">
                <label for="repartidor">
                    Repartidor
                </label>
                <select name="repartidor" id="repartidor"
                    class="w-[138px] h-[50px] rounded-[20px] p-2 border border-color-text">
                    <option value="" selected class="">Seleccione...</option>
                    @if ($empresa->usuarios && $empresa->usuarios->isNotEmpty())
                        @foreach ($empresa->usuarios->filter(fn($user) => $user->tipo === 'repartidor') as $item)
                            <option value="{{ $item->id }}">{{ $item->persona->nombres }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
            <div class="grid space-y-2">
                <label for="repartidor">
                    Buscar por Cliente
                </label>
                <div class="flex space-x-2">
                    <input type="text" placeholder="Buscar cliente..."
                        class="border border-color-text  w-[212px] h-[50px] p-2 rounded-[20px]">
                    <div
                        class="w-[50px] justify-center p-1 h-[50px] border items-center grid rounded-xl border-color-text">
                        <i class="fas fa-search   text-2xl "></i>
                    </div>
                </div>

            </div>

        </div>
        <div class="mt-8 grid md:grid-cols-3 grid-cols-1">
            @for ($i = 0; $i < 3; $i++)
                <div class="w-[368px] max-w[368px] max-h-[285px] h-[285px] text-base text-color-titulos-entrega">
                    <div
                        class="ml-[20px] grid  w-[338px] max-w[338px]  max-h-[255px] h-[255px] rounded-[20px] bg-white pl-[20px] pt-[27px]">
                        <div class="flex space-x-2">
                            <div class=""><svg class="w-[18px] h-[18px]" aria-hidden="true" viewBox="0 0 512 512"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M0 252.118V48C0 21.49 21.49 0 48 0h204.118a48 48 0 0 1 33.941 14.059l211.882 211.882c18.745 18.745 18.745 49.137 0 67.882L293.823 497.941c-18.745 18.745-49.137 18.745-67.882 0L14.059 286.059A48 48 0 0 1 0 252.118zM112 64c-26.51 0-48 21.49-48 48s21.49 48 48 48 48-21.49 48-48-21.49-48-48-48z">
                                    </path>
                                </svg></div>
                            <div class="">Pedido N° <b>558</b></div>
                        </div>
                        <div class="flex ">
                            <div class="mr-2"><svg xmlns="http://www.w3.org/2000/svg"
                                    xmlns:xlink="http://www.w3.org/1999/xlink" id="Capa_1" x="0px" y="0px"
                                    width="25px" height="25px" viewBox="0 0 388.638 382.53" xml:space="preserve">
                                    <g>
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                            d="M230.17,320.31V142.75h0.01c0-6.3,0.011-12.6-0.01-18.9  c-0.05-19.36-12.65-32.01-32-32.15c-3.74-0.03-7.48,0.1-11.21-0.1c-0.88-0.05-2.45-1.16-2.46-1.8c-0.16-9.56-0.11-19.12-0.11-29.04  c1.68-0.08,3-0.2,4.31-0.2c10.84-0.02,21.67,0.01,32.5-0.02c6.22-0.02,8.95-2.67,8.96-8.82c0-8.09,0.45-16.24-0.34-24.26  c-1.4-14.19-10.67-23.82-24.63-26.79c-0.601-0.12-1.15-0.44-1.721-0.67H74.22c-1.42,0.41-2.83,0.83-4.25,1.22  c-13.62,3.8-22.3,15.44-22.37,30.33c-0.09,18.41-0.03,36.82-0.03,55.23v4.91H36.08C16.91,91.7,7.24,99.04,2.17,117.44  c-0.07,0.22-0.28,0.4-0.42,0.6v238.34c0.31,0.91,0.67,1.8,0.93,2.71c3.42,12.141,11.18,19.78,23.52,22.67  c0.83,0.19,1.63,0.51,2.44,0.771h174.83c1.311-0.36,2.601-0.75,3.91-1.09c13.58-3.521,22.67-15.37,22.771-30.07  c0.069-10.351,0.06-20.71,0.05-31.061H230.17z M184.76,29.02c0.51-7.89,6.48-13.5,14.44-13.91c7.64-0.39,14.399,4.59,15.47,12.23  c0.76,5.38,0.521,10.91,0.45,16.37c-0.01,0.66-1.75,1.81-2.73,1.87c-4.1,0.21-8.21,0.08-12.319,0.08  c-4.11,0.01-8.21-0.06-12.32,0.04c-2.1,0.05-3.26-0.45-3.17-2.87C184.73,38.23,184.47,33.61,184.76,29.02z M62.3,32.02  c0-10.82,6.38-17.15,17.19-17.16c30.01-0.01,60.02,0,90.02,0h4.32c-5.57,9.52-4.04,19.49-4.08,29.22  c-0.11,29.63-0.1,59.27,0.04,88.9c0.02,3.57-0.94,4.59-4.55,4.57c-32.87-0.13-65.74-0.12-98.61-0.01  c-3.33,0.01-4.44-0.75-4.43-4.29C62.34,99.51,62.27,65.77,62.3,32.02z M215.28,125.38c0.01,74.44,0.01,148.89,0,223.33  c0,13.17-5.681,18.84-18.88,18.83c-53.78-0.06-107.561-0.13-161.331-0.2c-12.22-0.02-18.11-5.89-18.12-18.08  c-0.01-37.35,0-74.689,0-112.04c0-37.72,0.02-75.44-0.02-113.16c0-5.5,1.11-10.74,5.91-13.77c2.97-1.89,6.81-2.7,10.38-3.26  c3.4-0.53,6.96,0.04,10.43-0.16c2.87-0.17,3.59,0.99,3.56,3.69c-0.12,10.71-0.06,21.42-0.04,32.12c0.02,7.05,2.89,9.91,10.04,9.9  c39.21-0.07,78.42-0.15,117.63-0.24c6.96-0.01,9.72-2.75,9.74-9.74c0.05-10.71,0.12-21.42-0.05-32.12  c-0.05-3.16,1.04-4.14,4.08-3.96c3.84,0.23,7.72-0.04,11.57,0.12c8.431,0.35,14.391,6.21,15.04,14.63  C215.33,122.63,215.28,124.01,215.28,125.38z">
                                        </path>
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                            d="M55.057,274.141c2.49,0.005,4.991-0.15,7.466,0.038  c4.741,0.36,7.911,3.631,7.875,7.873c-0.034,4.099-3.039,7.452-7.615,7.714c-5.209,0.297-10.464,0.3-15.672-0.023  c-4.609-0.285-7.457-3.601-7.426-7.804c0.032-4.364,3.073-7.443,7.904-7.773C50.067,273.996,52.567,274.135,55.057,274.141z">
                                        </path>
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                            d="M55.024,335.561c-2.613,0-5.239,0.165-7.836-0.037  c-4.55-0.354-7.48-3.494-7.502-7.743c-0.022-4.12,2.609-7.351,7.091-7.647c5.444-0.361,10.949-0.37,16.392-0.002  c4.587,0.312,7.451,3.873,7.212,7.969c-0.243,4.154-3.337,7.161-7.893,7.45c-2.479,0.156-4.976,0.028-7.464,0.028  C55.024,335.572,55.024,335.566,55.024,335.561z">
                                        </path>
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                            d="M177.011,289.35c-2.852,0-5.728,0.226-8.552-0.055  c-3.99-0.395-6.543-3.437-6.55-7.287c-0.006-3.683,2.354-6.962,6.168-7.212c5.92-0.387,11.902-0.383,17.823-0.001  c3.806,0.245,6.207,3.535,6.214,7.189c0.008,3.821-2.594,6.747-6.545,7.358c-0.368,0.057-0.739,0.122-1.109,0.125  c-2.483,0.011-4.965,0.006-7.448,0.006C177.012,289.432,177.011,289.391,177.011,289.35z">
                                        </path>
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                            d="M115.849,289.37c-2.728,0-5.472,0.196-8.178-0.046  c-4.092-0.366-6.576-3.176-6.665-7.111c-0.091-4.008,2.144-7.164,6.238-7.446c5.792-0.398,11.649-0.395,17.443-0.018  c4.057,0.266,6.411,3.486,6.345,7.396c-0.067,4.017-2.732,6.786-6.999,7.267c-0.369,0.042-0.743,0.067-1.114,0.068  c-2.357,0.006-4.714,0.003-7.071,0.003C115.849,289.445,115.849,289.407,115.849,289.37z">
                                        </path>
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                            d="M115.648,59.37c-5.949,0-11.935,0.196-17.836-0.046  c-8.926-0.366-14.342-3.176-14.537-7.111c-0.198-4.008,4.677-7.164,13.605-7.446c12.632-0.398,25.408-0.395,38.045-0.018  c8.849,0.266,13.982,3.486,13.839,7.396c-0.146,4.017-5.958,6.786-15.266,7.267c-0.805,0.042-1.62,0.067-2.429,0.068  c-5.141,0.006-10.281,0.003-15.422,0.003C115.648,59.445,115.648,59.407,115.648,59.37z">
                                        </path>
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                            d="M115.648,88.704c-5.949,0-11.935,0.196-17.836-0.046  c-8.926-0.366-14.342-3.176-14.537-7.111c-0.198-4.008,4.677-7.164,13.605-7.446c12.632-0.398,25.408-0.395,38.045-0.018  c8.849,0.266,13.982,3.486,13.839,7.396c-0.146,4.017-5.958,6.786-15.266,7.267c-0.805,0.042-1.62,0.067-2.429,0.068  c-5.141,0.006-10.281,0.003-15.422,0.003C115.648,88.779,115.648,88.741,115.648,88.704z">
                                        </path>
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                            d="M176.964,320.352c2.607-0.003,5.22-0.115,7.819,0.023  c4.262,0.228,7.072,2.99,7.313,6.977c0.228,3.74-2.212,7.329-6.15,7.618c-5.914,0.436-11.905,0.44-17.821,0.022  c-3.975-0.281-6.377-3.768-6.201-7.568c0.186-4.002,2.968-6.804,7.221-7.043C171.743,320.234,174.356,320.354,176.964,320.352z">
                                        </path>
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                            d="M115.969,320.349c2.606-0.002,5.217-0.112,7.816,0.023  c4.152,0.219,6.88,2.797,7.225,6.611c0.366,4.049-1.949,7.669-6.074,7.986c-5.908,0.454-11.902,0.462-17.811,0.016  c-4.201-0.317-6.415-3.764-6.103-7.933c0.288-3.843,2.977-6.447,7.131-6.675C110.752,320.236,113.364,320.352,115.969,320.349z">
                                        </path>
                                    </g>
                                    <path fill-rule="evenodd" clip-rule="evenodd" fill="#293241"
                                        d="M187.676,212.25c0,6.627-5.373,12-12,12H56.361 c-6.627,0-12-5.373-12-12v-22.5c0-6.627,5.373-12,12-12h119.315c6.627,0,12,5.373,12,12V212.25z">
                                    </path>
                                    <g>
                                        <path fill="#293241"
                                            d="M291.718,127.335c-23.3,0-44.699,8.26-61.43,22.01v20.79c15.01-17.04,36.98-27.8,61.43-27.8  c45.17,0,81.92,36.74,81.92,81.91s-36.75,81.92-81.92,81.92c-24.439,0-46.42-10.76-61.43-27.8v20.779  c16.73,13.761,38.13,22.021,61.43,22.021c53.44,0,96.92-43.479,96.92-96.92C388.638,170.805,345.158,127.335,291.718,127.335z">
                                        </path>
                                    </g>
                                    <g>
                                        <path fill="#293241"
                                            d="M289.723,243.598c0-2.606-0.923-4.638-2.768-6.094c-1.846-1.455-5.087-2.962-9.725-4.52  c-4.639-1.557-8.43-3.063-11.375-4.52c-9.581-4.705-14.371-11.172-14.371-19.398c0-4.096,1.193-7.71,3.58-10.842  c2.387-3.131,5.764-5.568,10.131-7.313c4.367-1.743,9.275-2.615,14.727-2.615c5.314,0,10.08,0.948,14.295,2.844  c4.215,1.896,7.49,4.596,9.826,8.1s3.504,7.508,3.504,12.01h-17.773c0-3.013-0.923-5.349-2.768-7.008  c-1.846-1.658-4.342-2.488-7.49-2.488c-3.183,0-5.696,0.703-7.541,2.107c-1.846,1.405-2.768,3.191-2.768,5.357  c0,1.896,1.016,3.614,3.047,5.154c2.031,1.541,5.603,3.132,10.715,4.773c5.111,1.643,9.31,3.411,12.594,5.307  c7.989,4.604,11.984,10.952,11.984,19.043c0,6.467-2.438,11.545-7.313,15.234c-4.875,3.69-11.562,5.535-20.059,5.535  c-5.992,0-11.418-1.074-16.275-3.225c-4.858-2.149-8.515-5.095-10.969-8.836c-2.455-3.74-3.682-8.049-3.682-12.924h17.875  c0,3.961,1.023,6.881,3.072,8.76c2.048,1.879,5.374,2.818,9.979,2.818c2.945,0,5.272-0.635,6.982-1.904  C288.867,247.686,289.723,245.9,289.723,243.598z">
                                        </path>
                                        <path fill="#293241"
                                            d="M321.766,269.598h-12.137l25.238-80.285h12.188L321.766,269.598z">
                                        </path>
                                    </g>
                                </svg></div>
                            <div class="jet-listing-dynamic-field__content"> S/5.00</div>
                            <div class="flex items-start ml-6"><img decoding="async"
                                    src="{{ asset('imagenes/efectivo.svg') }}" alt="Pagó en Efectivo"
                                    style="width: 25px;height: auto;margin-right: 5px;vertical-align: middle">
                                <span><b>Efectivo</b></span>
                            </div>

                        </div>

                        <div class="flex space-x-2">
                            <div class="jet-listing-dynamic-field__icon is-svg-icon"><svg class="w-[18px] h-[18px]"
                                    aria-hidden="true" viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M313.6 304c-28.7 0-42.5 16-89.6 16-47.1 0-60.8-16-89.6-16C60.2 304 0 364.2 0 438.4V464c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48v-25.6c0-74.2-60.2-134.4-134.4-134.4zM400 464H48v-25.6c0-47.6 38.8-86.4 86.4-86.4 14.6 0 38.3 16 89.6 16 51.7 0 74.9-16 89.6-16 47.6 0 86.4 38.8 86.4 86.4V464zM224 288c79.5 0 144-64.5 144-144S303.5 0 224 0 80 64.5 80 144s64.5 144 144 144zm0-240c52.9 0 96 43.1 96 96s-43.1 96-96 96-96-43.1-96-96 43.1-96 96-96z">
                                    </path>
                                </svg></div>
                            <div class="jet-listing-dynamic-field__content">Sandra Maribel Salas Tuanama</div>
                        </div>
                        <div class="flex ">
                            <div class="mr-2"><svg class="w-[18px] h-[18px]" aria-hidden="true" viewBox="0 0 448 512"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M400 64h-48V12c0-6.627-5.373-12-12-12h-40c-6.627 0-12 5.373-12 12v52H160V12c0-6.627-5.373-12-12-12h-40c-6.627 0-12 5.373-12 12v52H48C21.49 64 0 85.49 0 112v352c0 26.51 21.49 48 48 48h352c26.51 0 48-21.49 48-48V112c0-26.51-21.49-48-48-48zm-6 400H54a6 6 0 0 1-6-6V160h352v298a6 6 0 0 1-6 6zm-52.849-200.65L198.842 404.519c-4.705 4.667-12.303 4.637-16.971-.068l-75.091-75.699c-4.667-4.705-4.637-12.303.068-16.971l22.719-22.536c4.705-4.667 12.303-4.637 16.97.069l44.104 44.461 111.072-110.181c4.705-4.667 12.303-4.637 16.971.068l22.536 22.718c4.667 4.705 4.636 12.303-.069 16.97z">
                                    </path>
                                </svg></div>
                            <div class="jet-listing-dynamic-field__content">18/01/2025</div>
                            <div class="flex space-x-2 ml-5">
                                <div class="jet-listing-dynamic-field__icon is-svg-icon"><svg class="w-[18px] h-[18px]"
                                        aria-hidden="true" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8zm0 448c-110.5 0-200-89.5-200-200S145.5 56 256 56s200 89.5 200 200-89.5 200-200 200zm61.8-104.4l-84.9-61.7c-3.1-2.3-4.9-5.9-4.9-9.7V116c0-6.6 5.4-12 12-12h32c6.6 0 12 5.4 12 12v141.7l66.8 48.6c5.4 3.9 6.5 11.4 2.6 16.8L334.6 349c-3.9 5.3-11.4 6.5-16.8 2.6z">
                                        </path>
                                    </svg></div>
                                <div class="jet-listing-dynamic-field__content">12:06:53 am</div>
                            </div>
                        </div>

                        <div class="flex space-x-2">
                            <div class="jet-listing-dynamic-field__icon is-svg-icon"><svg class="w-[18px] h-[18px]"
                                    aria-hidden="true" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M288 39.056v16.659c0 10.804 7.281 20.159 17.686 23.066C383.204 100.434 440 171.518 440 256c0 101.689-82.295 184-184 184-101.689 0-184-82.295-184-184 0-84.47 56.786-155.564 134.312-177.219C216.719 75.874 224 66.517 224 55.712V39.064c0-15.709-14.834-27.153-30.046-23.234C86.603 43.482 7.394 141.206 8.003 257.332c.72 137.052 111.477 246.956 248.531 246.667C393.255 503.711 504 392.788 504 256c0-115.633-79.14-212.779-186.211-240.236C302.678 11.889 288 23.456 288 39.056z">
                                    </path>
                                </svg></div>
                            <div class="text-wrap">Actualizado por Sandra Maribel Salas Tuanama</div>
                        </div>
                    </div>
                </div>
            @endfor
        </div>

        <p class="text-base flex justify-center">No se encontraron pedidos pagados en el día de hoy.
        </p>

    </div>
@endsection
